...
while getopts ":s:o:r:P:dvL" opt; do
    case ${opt} in
        s )
            SCALE=$OPTARG
            ;;
...
function Load() {
    ...
    CPU_IDL=$(echo "(1-$CPU_UTL)*$CPU_CNT" | bc | awk -F. '{print$1}')
    CPU_USE=$(echo $(($CPU_IDL<$MAXPROCS?$CPU_IDL:$MAXPROCS)))
    #chunks=$(echo "($SCALE/$FREE_MEM+1)*$CPU_USE" | bc)
    chunks=$(echo "$SCALE*0.0752+2" | bc | awk -F. '{print$1}')
    if [ $chunks -lt 1 ]; then
